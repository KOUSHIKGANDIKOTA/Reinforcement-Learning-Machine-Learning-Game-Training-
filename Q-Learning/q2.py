...
def update_q_values(state, action, reward, new_state):
    ...
    max_next_q = max(q_table[new_state])
    new_q = reward + discount_factor * max_next_q
    q_table[state][action] = current_q + learning_rate * (new_q - current_q)
...
for episode in range(training_episodes + evaluate_after):
    ...
    update_q_values(state, action, reward, new_state)
